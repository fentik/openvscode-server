...
const viewDescriptor: IJSONSchema = {
	...
	properties: {
		...
		initialSize: {
			type: 'number',
			description: localize('vscode.extension.contributes.view.size', "The initial size of the view. The size will behave like the css 'flex' property, and will set the initial size when the view is first shown. In the side bar, this is the height of the view. This value is only respected when the same extension owns both the view and the view container."),
		},
		order: {
			type: 'number',
			...
			Object.entries(value).forEach(([key, value]) => {
				...
				for (let index = 0; index < value.length; index++) {
					...
					const viewDescriptor = <ICustomTreeViewDescriptor>{
						...
						canMoveView: item.canMoveView !== undefined ? item.canMoveView : viewContainer?.id !== REMOTE,
